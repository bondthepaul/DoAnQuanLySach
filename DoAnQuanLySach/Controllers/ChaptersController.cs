...
namespace DoAnQuanLySach.Controllers
{
    public class ChaptersController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ChapterId,ChapterName,ShortContent,BookId")] Chapter chapter)
        {
            if (ModelState.IsValid)
            {
                var list = db.Chapters;
                int tmp = 0;
                for (int i = 1; i <= list.ToList().Count; i++)
                {
                    Chapter user = db.Chapters.Find(i);
                    if (user == null) tmp = i;
                }
                chapter.ChapterId = tmp;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ChapterId,ChapterName,ShortContent,BookId")] Chapter chapter)
        {
            if (ModelState.IsValid)
            {
